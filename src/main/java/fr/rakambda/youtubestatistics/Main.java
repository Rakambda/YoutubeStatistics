package fr.rakambda.youtubestatistics;

import fr.rakambda.youtubestatistics.cli.CLIParameters;
import fr.rakambda.youtubestatistics.settings.Settings;
import fr.rakambda.youtubestatistics.settings.YouTubeChannel;
import fr.rakambda.youtubestatistics.utils.YouTubeChannelCollector;
import fr.rakambda.youtubestatistics.utils.YouTubeHelper;
...
public class Main{
	...
	public static void main(final String[] args){
		...
		try{
			...
			parser.getUserId().or(() -> youtube.getOwner()
					.map(YouTubeChannel::getChannelId))
					.ifPresentOrElse(channelId -> {
						...
						userConfiguration.getChannels().values().stream()
								.sorted(parameters.getSorting())
								.limit(parameters.getBestChannelCount())
